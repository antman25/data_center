...
chown consul:consul /etc/certs/${SERVER_NAME}.cert.pem
chmod 444 /etc/certs/${SERVER_NAME}.cert.pem

cp /root/ca/intermediate/private/${SERVER_NAME}.key.pem /etc/certs
chown consul:consul /etc/certs/${SERVER_NAME}.key.pem
chmod 400 /etc/certs/${SERVER_NAME}.key.pem
